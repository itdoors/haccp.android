...
import com.itdoors.haccp.fragments.PointDetailsFragment;
import com.itdoors.haccp.fragments.StatisticsFragment;
import com.itdoors.haccp.fragments.StatisticsFragment.MODE;
import com.itdoors.haccp.fragments.TimeRangeDialogFragment;
...
public class PointDetailsActivity extends SherlockFragmentActivity implements StatisticsFragment.OnContextMenuItemPressedListener,
																			  StatisticsFragment.OnLongStatisticsItemPressedListener,
																			  StatisticsFragment.TimeRangeParametersHolder,																			  
																			  StatisticsFragment.StatisticsListModeHolder,
	...
	private StatisticsFragment.MODE mStatisticFragmentMode;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.control_point_frame);
	...
	private final View.OnClickListener retryList = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			LoadActivityUtils.removeErrorView(PointDetailsActivity.this);
			LoadActivityUtils.addLoadingView(PointDetailsActivity.this);
	...
	protected void fillFragments(){
		...
		if(mContent != null){
			...
			if(mControlPoinDetailsFragment != null)
				((PointDetailsFragment) mControlPoinDetailsFragment).fillUI(content);
			//if(mControlpointShortInfoFragment != null)
	...
	protected void changeStatusOnAttributes(PointStatus status){
		if(mControlPoinDetailsFragment != null){
			((PointDetailsFragment) mControlPoinDetailsFragment).changeStatusOnAttributes(status);
		}
	}
	protected void updateStatisticsAfterAddRequestSuccess(){
		if(mContent != null){
			final Content content = (Content) mContent;
			if(mControlPoinDetailsFragment != null){
				((PointDetailsFragment) mControlPoinDetailsFragment).updateStatisticsAfterAddRequestSuccess(content);
			}
		}
	}
	...
	protected void updateStatisticsAfterTimeRangeLoadSuccess(){
		if(mContent != null){
			final Content content = (Content) mContent;
			if(mControlPoinDetailsFragment != null){
				((PointDetailsFragment) mControlPoinDetailsFragment).updateStatisticsAfterTimeRangeLoadSuccess(content);
			}
		}
	}
	...
	protected void updateStatisticsAfterRefreshSuccess(){
		if(mContent != null){
			final Content content = (Content) mContent;
			if(mControlPoinDetailsFragment != null){
				((PointDetailsFragment) mControlPoinDetailsFragment).updateStatisticsAfterRefreshSuccess(content);
			}
		}
	}
	protected void refreshReshFailed(){
		if(mControlPoinDetailsFragment != null){
			((PointDetailsFragment) mControlPoinDetailsFragment).refreshReshFailed();
		}
	}
	
	protected void scrollStatisticsToBottom(){
		if(mControlPoinDetailsFragment != null){
			((PointDetailsFragment) mControlPoinDetailsFragment).scroolStatisticsToBottom();
		}
	}
	protected void scrollStatisticsToTop(){
		if(mControlPoinDetailsFragment != null){
			((PointDetailsFragment) mControlPoinDetailsFragment).scroolStatisticsToTop();
		}
	}
	...
	private void onInitLoadFinished( RESTLoader.RESTResponse data){
        ...
        if (code == 200 && !json.equals("")) {
        	...
        	try {
				...
				mContent = content;
				//remove loading view
				LoadActivityUtils.removeLoadingView(PointDetailsActivity.this);
				//Show result in short and details fragments
				fillFragments();
        ...
        if(failed){
        	...
        	LoadActivityUtils.removeLoadingView(PointDetailsActivity.this);
        	LoadActivityUtils.addErrorView(PointDetailsActivity.this, retryList);
