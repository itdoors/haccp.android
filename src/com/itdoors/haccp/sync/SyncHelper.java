...
public class SyncHelper {
    ...
    /**
     * 1) In initial setup loading zipped json db representation, unzipping it
     * and parsing to sqlite. 2) All actions, that need to be queued and
     * performed in sync, saves in "transactions" table, from where they are
     * taken and handled by this adapter 3) Server to device sync happens when
     * it need and also performed here.
     * 
    ...
    public void performSync(SyncResult syncResult, int flags) throws IOException {
        ...
        if ((flags & FLAG_SYNC_INITIAL) != 0) {
            ...
            if (localParse) {
                ...
                try {
                    ...
                    String token = SyncUtils.getAccessToken(mContext);
                    String uri = DB_URI + "?" + Constants.ACCESS_TOKEN_PARM + "=" + token;
                    Logger.Logd(getClass(), "uri:" + uri);

                    HttpHelper.downloadViaHttp(uri, tempFile, HttpHelper.ContentType.GZIP);
                    Logger.Logi(TAG, "Loading took "
                    ...
                    try {
                        ...
                        Logger.Logi(getClass(),
                                "Perform parsing, file :" + tempFile.getAbsolutePath());
                        db.beginTransaction();
                        StreamParser.parseZippedDatabase(tempFile, db);
                        db.setTransactionSuccessful();

                        if (syncResult != null) {
                            ++syncResult.stats.numUpdates; // TODO: better way
                                                           // of indicating
                                                           // progress?
                    ...
                    Logger.Logi(getClass(), "Removing temp file :" + path + ", with exeption  - "
                            + (exeption ? "yes" : "no"));
                }

                Logger.Logi(TAG, "Local sync took " + (System.currentTimeMillis() - startLocal)
                        + "ms");
                if (sucess) {
        ...
        if ((flags & FLAG_SYNC_REMOTE) != 0) {
            ...
            // device to server
            {
                ...
                Cursor cursor = cr.query(HaccpContract.Transactions.PENDING_TRANSACTIONS_URI,
                        PendingTransactionsQuery.PROJECTION, null, null, null);
                ...
                while (!cursor.isAfterLast()) {
                    ...
                    // If the row still exists in transactions table and is
                    // still pending, it will be
                    // updated to transacting in-progress.

                    Uri updaterequestUri = HaccpContract.Transactions
                            .buildInProgressUriForId(requestId);
                    ...
                    if (count > 0) {
                        ...
                        if (action == RestAction.INSERT_STATISTICS) {
                            if (method == HttpMethod.POST) {
                                restCommand = new InsertStatisticsCommand(mContext, requestId, uri,
                                        params);
                                Logger.Logi(getClass(), "create new insert statistics command: "
                                        + restCommand.toString());
                            }
                            else {
                                Logger.Loge(getClass(),
                                        "invalid request method for insert statistics action : "
                                                + method.toString());
                                syncResult.databaseError = true;
                                clearTransacting(requestId);
                                continue;
                            ...
                            if (method == HttpMethod.POST) {
                                restCommand = new UpdatePointStatusCommand(mContext, requestId,
                                        uri, params);
                                Logger.Logi(getClass(), "create new update point status action : "
                                        + method.toString());
                            }
                            else {
                                Logger.Loge(getClass(),
                                        "invalid request method for update point status action : "
                                                + method.toString());
                                syncResult.databaseError = true;
                                clearTransacting(requestId);
                                continue;
            ...
            // server to device
            {
                ...
                if (queryTransInfo.isRefreshOutstanding(true, mContext)) {
                    /*
                     * queryTransInfo.markInProgress(); restCommand = new
                     * UpdateDatabaseCommand(); handleSync(restCommand,
                     * syncResult);
                     */
                }
            }
    ...
    /**
     * Sync the local database with the web service by executing the RESTCommand
     * which will call the REST API. Hard errors will cancel the sync operation.
     ...
     * @return True if the call to the REST API was successful. True if the call
     *         fails and request should be retried. False if the call fails and
    ...
    /**
     * Used to clear the transacting flags if the request fails before the call
     * to the REST API. We will hopefully never get here, but reason for this
     * most likely bad data in input.
     * 
