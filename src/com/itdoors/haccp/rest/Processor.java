...
public class Processor {
    ...
    /**
     * Read request. Mark transaction completed. Make changes in database.
     * Remove transaction.
     */
    ...
    public void handleResponce(Responce responce) {
        ...
        markCompletedValues.put(HaccpContract.Transactions.TRANSACTING,
        ...
        switch (action) {
            case INSERT_STATISTICS: {
                ...
                if (record != null) {
                    ...
                    final String pointID = record.getPointId();
                    ...
                    statisticsInsertValues.put(HaccpContract.Statistics.CHARACTERISTICS_ID,
                            record.getGroupCharacteristicsId());
                    if (record.getCreatedAt() != null)
                        statisticsInsertValues.put(HaccpContract.Statistics.CREATED_AT,
                                Long.toString(record.getCreatedAt().getTime() / 1000));
                    if (record.getEntryDate() != null)
                        statisticsInsertValues.put(HaccpContract.Statistics.ENTRY_DATE,
                                Long.toString(record.getEntryDate().getTime() / 1000));
            ...
            case UPDATE_POINT_STATUS: {
                ...
                if (record != null) {
                    ...
                    final String pointId = record.getId();
            ...
            default:
                throw new UnsupportedOperationException(
                        "unknown action on Processor.handleResponce : " + action.toString());
        }

        // remove transaction
        Logger.Logi(getClass(), "remove transaction " + requestId);
        ContentProviderOperation removeTransaction = ContentProviderOperation.newDelete(
                transactionUri).build();
    ...
    /**
     * Check try_count from database against MAX_REQUEST_ATTEMPTS. If try_count
     * < MAX_REQUEST_ATTEMPTS then increment tryCount. If try_count >=
     * MAX_REQUEST_ATTEMPTS then delete transaction and show notification.
     * 
     * @param requestId The _ID column from the table to update.
     * @param result The HttpStatus code.
     * @param allowRetry If true and less then MAX_REQUEST_ATTEMPTS increment
     *            retry counter and try again later. If false then mark
     *            transaction as completed.
     */
    public boolean requestFailure(long requestId, int result, boolean allowRetry) {

        Logger.Logi(getClass(), "requestFailure requestId: " + requestId + ", " + "result: "
        ...
        if (allowRetry && tryCount < SyncAdapter.MAX_RETRY_COUNT_FOR_TRANSACTION) {
            ...
            Logger.Loge(getClass(), "processing request failure. requestId :" + requestId
                    + ", set retry tryCount=" + tryCount);
            ...
            Logger.Loge(getClass(), "remove failed transaction. requestId : " + requestId + "."
                    + causeMsg);
