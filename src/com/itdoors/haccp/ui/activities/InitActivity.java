...
public class InitActivity extends BaseSherlockFragmentActivity {
    ...
    private static final String POST_AUTH_CATEGORY = "com.itdoors.haccp.POST_AUTH";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (activeNetwork == null || !activeNetwork.isConnected()) {
            ToastUtil.ToastLong(getApplicationContext(),
                    getResources().getString(R.string.no_connection_cant_sync));
        }
    ...
    public void onResume() {
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(syncFinishedReceiver,
                new IntentFilter(SyncComplete.ACTION_FINISHED_SYNC));
    ...
    public static class RetryInitSyncEvent {

    }

    public static class InitSyncFailedEvent {

    }

    public void onEventMainThread(RetryInitSyncEvent event) {
        // Start sync again
        SyncUtils.requestManualSync(getApplicationContext());
    }
    ...
    private BroadcastReceiver syncFinishedReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            boolean localInsertSuccesfull = intent.getExtras().getBoolean(
                    Intents.SyncComplete.LOCAL_SYNC_SUCCESFULLY);
            if (localInsertSuccesfull) {
                finishInitialSetup();
            }
            else {

                EventBus.getDefault().postSticky(new InitSyncFailedEvent());
                Logger.Loge(getClass(), "Hard error has happened when performing initial sync.");

                ToastUtil.ToastLong(InitActivity.this.getApplicationContext(),
                        getString(R.string.initial_setup_db_failed_error));
            }
