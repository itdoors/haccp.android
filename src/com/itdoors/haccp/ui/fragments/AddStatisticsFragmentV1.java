...
public class AddStatisticsFragmentV1 extends SherlockFragment implements LoaderCallbacks<Cursor>{
	...
	*/
	...
	private Uri getPointInfoUri(){
		if(getActivity()!= null)
			 return HaccpContract.Points.buildPointUri(getActivity().getIntent().getIntExtra(Intents.Point.UID, -1));
		else throw new IllegalStateException(" fragment is not Attached to activity");
	}
	...
	private void clearStatuses(){
		if(getView()!=null){
			...
			RadioGroup mRadioGroup = (RadioGroup)getView().findViewById(R.id.add_st_statuses_radio_group);
	...
	private void clearCharacteristics(){
		if(getView() != null){
			...
			LinearLayout mLinearLayout = (LinearLayout)getView().findViewById(R.id.add_st_char_fields_holder);
	...
	private void onGroupCharacteristicsLoadFinished(Cursor cursor){
  		...
  		while (!cursor.isAfterLast()) {
			...
			int criticalValueBottom = cursor.getInt(GroupCharacteristicsQuery.CRITICAL_VALUE_BOTTOM);
			...
			catch(IllegalArgumentException wrongArgumentException ){wrongArgumentException.printStackTrace();}
			
			GroupCharacteristic groupCharacteristics = new GroupCharacteristic(uid, name, unit, null, max, min, criticalValueBottom, criticalValueTop);
	...
	private void setStatusesRadioGroup(LayoutInflater inflater, ViewGroup container, ArrayList<PointStatus> statuses){
	...
	private void setCharacteristicsViews(LayoutInflater inflater, ViewGroup container, List<GroupCharacteristicField> characteristicFields){
